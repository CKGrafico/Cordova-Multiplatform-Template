...
module $safeprojectname$ {
    ...
    function onDeviceReady() {
        angular.module('$safeprojectname$', ['ionic'])
            .controller('navigationController', $safeprojectname$.NavigationController)
            .controller('actionsController', $safeprojectname$.ActionsController)
            .config(['$stateProvider', '$urlRouterProvider', states])
            .config(['$compileProvider', function ($compileProvider) {
			$compileProvider.aHrefSanitizationWhitelist(/^\s*(https?|file|mailto|ms-appx):/);
		}]);

        angular.bootstrap(document.querySelector('body'), ['$safeprojectname$']);
    }
